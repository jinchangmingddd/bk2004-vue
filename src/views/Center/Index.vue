<template>
    <div class="center-view">
        ...
        >
            ...
            />
            <img class="avator-icon" src="https://dss2.bdstatic.com/70cFvnSh_Q1YnxGkpoWK1HF6hhy/it/u=2204428928,2747861962&fm=26&gp=0.jpg" v-if="userInfo.mobile" alt="" style="width: 50px">
            <div class="nick-name" @click="toLogin" v-if="!userInfo.mobile">
                立即登录
            </div>
        ...
        >
            ...
            />
            <span class="label" @click="my_balence">余额</span>
            <span class="value"
...
export default {
    ...
    created(){
        // 获取用户信息
        // 判断state是否存在jwt,发送请求，没有jwt说明没有用户信息，不用发请求，让用户自己登陆，存jwt
        let jwt = this.$store.state.global._token;
        if(jwt){
            // 发送请求   有jtw需要带着jwt  ，使用请求拦截
            this.$http.get("http://127.0.0.1:2004/backend/get_user_info").then(ret=>{
                console.log(ret);
